...
import { session, Markup, Telegraf, Context } from 'telegraf';
...
export class TelegramService {
  ...
  async handleStart(ctx: Context) {
    ...
    const checkUser = await this.walletService.findOneUser(
    ...
    if (!checkUser) {
      ...
      const message = await ctx.replyWithHTML(
        `Xin chào <a href="tg://user?id=${userInfo.userId}">@${userInfo.username}</a>!!\n💳Địa chỉ wallet!\n<code>${checkUser.address}</code>\n
...
🎟️Nạp thêm <b>PGX</b> <a href="https://faucet.miraichain.io/"><u>click here</u>!</a>`,
  ...
  async handleToken(msg: Context, userInfo: UserInfo, data: DataCache) {
    const user = await this.walletService.findOneUser(
      userInfo.userId.toString(),
    );
    if (!user) {
    ...
    const tokensArrayButton = [
      Markup.button.callback('HUSD', Button.HUSD),
      Markup.button.callback('MTK', Button.MTK),
    ];
    tokens.forEach((token) => {
      tokensArrayButton.push(
        Markup.button.callback(token.symbol, token.symbol),
      );
    });
  ...
  async handleButton(msg: Context) {
    ...
    switch (userInfo.text) {
      ...
      default:
        ...
        messages.push(
          await msg.reply('Tôi chỉ thực hiện được như bên dưới thôi!'),
        );
  ...
  //Action Handler
  async handleDepositAction(msg: Context, userInfo: UserInfo, data: DataCache) {
    if (data.step === 1) {
  ...
  async validateDepositAmount(
    ...
    if (!Number(userInfo.text)) {
      const message = await msg.reply(
        'Số tiền sai cú pháp, Vui lòng nhập lại! Để hủy thao tác nhập lệnh /cancel',
      );
      this.deleteBotMessage(message, 5000);
      return false;
    }
  ...
  async createDepositTransaction(
    ...
    const address = await this.walletService.checkAddress(
      userInfo.userId.toString(),
    );
  ...
  async executeDepositAction(
    ...
    //execute mint token
    const transactionHash = await this.walletService.mintTokens(
    ...
    if (transactionHash === TokenStatus.NOT_SUPPORT) {
      messages.push(
        await msg.reply(
          `Token hiện tại không support mint! Vui lòng thực hiện hành động khác`,
        ),
      );
      this.cacheManager.del(userInfo.userId.toString());
      await this.handleStart(msg);
  ...
  async handleWithDrawAction(
    ...
    if (data.step === 1) {
      ...
      if (data.step === 2) {
        ...
        const privateKey = await this.walletService.checkPrivateKeyByID(userId);
        await this.transactionService.updateTransactionState(
          TransactionStatus.PENDING,
          transaction.id,
        );
        const message = await msg.reply(`processing....`);
        messages.push(message);
        const transactionHash = await this.walletService.burn(
          data.money,
          privateKey,
        );
        if (transactionHash === TokenStatus.NOT_SUPPORT) {
          messages.push(
            await msg.reply(
              `Token hiện tại không support burn! Vui lòng thực hiện hành động khác`,
            ),
          );
          this.cacheManager.del(userInfo.userId.toString());
          await this.handleStart(msg);
        ...
        if (!transactionHash) {
          ...
          messages.push(
            await msg.replyWithHTML(
              `Lượng token PGX hiện tại không đủ để thực hiện giao dịch`,
            ),
          );
          this.deleteBotMessages(messages, 5000);
          return;
  ...
  async handleReplaceWalletAction(
    msg: Context,
    userInfo: UserInfo,
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    data: DataCache,
  ...
  async handleTransferByAddressAction(
    msg: Context,
    userInfo: UserInfo,
    data: DataCache,
  ) {
  ...
  async handleSendMoneyAction(
    ...
    if (data.action === Action.SEND_MONEY_ADDRESS) {
      ...
      const token = (await this.walletService.findOneUser(userId))
        .currentSelectToken;
      const defaultTxHash = 'Unavailable';
      const createTransaction = {
        balance: String(data.money),
      ...
      if (
        Object(checkStatus).status === TransactionStatus.SUCCESS &&
        data.step === 2
      ) {
        await this.transactionService.updateTransactionState(
          TransactionStatus.SUCCESS,
        ...
        const message = await msg.replyWithHTML(
          `Lượng token PGX hiện tại không đủ để thực hiện giao dịch hoặc có lỗi ngoài dự kiễn đã xảy ra`,
        );
        this.deleteBotMessage(message, 5000);
        messages.push(message);
        await this.transactionService.updateTransactionState(
  ...
  async handleForgotPasswordAction(
    msg: Context,
    userInfo: UserInfo,
    data: DataCache,
  ) {
    ...
    if (data.action === Action.FORGOT_PASSWORD && data.step === 1) {
      const isVerified = await this.walletService.verifyBackupPhrase(
        userInfo.text,
        wallet.address,
      );
      if (isVerified) {
  ...
  async handleAddTokenAction(msg: any, userInfo: UserInfo, data: DataCache) {
    const checkAdress = await this.walletService.checkAddressContract(
      userInfo.text,
    );
    if (!checkAdress) {
    ...
    const newToken = await this.tokenService.handleNewToken(
      userInfo.text,
      userInfo.userId.toString(),
    );
    if (newToken) {
  ...
  async handleDeleteTokenAction(
    msg: Context,
    userInfo: UserInfo,
    data: DataCache,
  ) {
    const token = userInfo.text;
    const deleteToken = await this.tokenService.removeTokenFromList(
      token,
      userInfo.userId.toString(),
    );
    if (deleteToken) {
  ...
  async handleImportAccountButton(
    msg: Context,
    data: DataCache,
    userInfo: UserInfo,
  ) {
  ...
  async handleImportAction(msg: Context, userInfo: UserInfo, data: DataCache) {
    ...
    if (data.action == Action.IMPORT && data.step === 1) {
      ...
      const encryptedPrivateKey = await this.walletService.encryptPrivateKey(
        userInfo.text,
      );
      const createWallet = {
        userId: user.userId,
      ...
      const isCreated = await this.walletService.createWallet(
        createWallet,
        address,
      );
      if (!isCreated) {
      ...
      const message = await msg.reply(`Vui lòng nhập mật khẩu mới cho ví!`);
      this.deleteBotMessage(message, 10000);
    }

    if (data.action == Action.IMPORT && data.step === 2) {
      await msg.reply(`Import ví cho user ${user.userId}...`);
      const update = this.walletService.updatePassword(
        userInfo.text,
        user.userId.toString(),
      );
      if (update) {
        await msg.reply(`Import thành công!`);
        await this.handleStart(msg);
        await this.cacheManager.del(userInfo.userId.toString());
      } else {
        await msg.reply(`Lỗi`);
  ...
  async handleCreateAccountAction(
    msg: Context,
    userInfo: UserInfo,
    data: DataCache,
  ) {
    ...
    const message = await msg.replyWithHTML(
      `Recovery phrase dùng phòng khi bạn quên mật khẩu hãy lưu lại ở đâu đó trong máy bạn: \n <code>${wallet.mnemonic}</code>`,
      this.deleteButton,
    );
    this.processMessages.set(message.message_id, message);
    const createWallet = {
    ...
    messages.push(await msg.reply(`Tạo tài khoản thành công!`));
    await this.handleStart(msg);
    await this.cacheManager.del(userInfo.userId.toString());
    await this.deleteBotMessages(messages, 5000);
  }
  async handleCreateAccountButton(
    ...
    if (checkUser) {
      messages.push(
        await msg.reply(`Đã có tài khoản! Vui lòng thực hiện hành động khác`),
      );
      await this.handleStart(msg);
    }
    if (data.action === '') {
  ...
  async handleForgotPasswordButton(
    msg: Context,
    data: DataCache,
    userInfo: UserInfo,
    checkUser: WalletEntity,
  ...
  async handleDeleteTokenButton(
    ...
    if (data.action === '') {
      ...
      const finalMessage = await msg.reply(
        'Hãy nhập vào token contract address cần xóa:',
      );
      this.deleteBotMessage(finalMessage, 10000);
    }
  }
  ...
  async handlePrivateKeyButton(
    ...
    if (data.action === '') {
      ...
      messages.push(
        await msg.reply(
          `Enter your password to see your private key: `,
          this.forgotPassword,
        ),
      );
    } else {
  ...
  async handlePrivateKeyAction(
    ...
    const userId = userInfo.userId.toString();
    const hashPassword = (await this.walletService.findOneUser(userId))
      .password;
    const isPassword = await this.walletService.verifyPassword(
      userInfo.text,
      hashPassword,
    );
    if (isPassword) {
    ...
    const tempMessage = await msg.reply(
      'Wrong Password! Try again (to cancel action type /cancel)',
    );
    this.deleteBotMessage(tempMessage, 5000);
  }
  ...
  async handleDeleteButton(msg: Context) {
    const messageId =
      msg.callbackQuery?.message.message_id || msg.message?.message_id;
