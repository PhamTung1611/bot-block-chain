    ...
    switch (data.action) {
      ...
      case Action.WITHDRAW:
        if (data.step === 1) {
          ...
          if (data.step === 2) {
            await this.cacheManager.set(options.idUser, data, 30000);
            const checkWithDrawn = await this.wallerService.withdrawn(
              options.idUser,
              Number(data.money),
            );
            ...
            if (checkWithDrawn === WalletStatus.NOT_FOUND) {
              await this.cacheManager.del(options.idUser);
              await msg.reply(
                `tài khoản hoặc ví không tồn tại, vui lòng thử lại`,
                this.keyboardMarkup,
              );
              break;
            }
            ...
            try {
              ...
              console.log(err);
              await this.cacheManager.del(options.idUser);
              await msg.reply(`Rút tiền thất bại...`, this.keyboardMarkup);
            }
            const createTransaction = {
              balance: String(data.money),
              type: String(data.action),
